		...
		if sub == "Sales by Region":
			...
			if len(region_cols) >= 1:
				...
				if not agg.empty:
					...
					sns.barplot(x=agg.values, y=labels, ax=ax)
					ax.set_xlabel("Total Sales")
			...
			if model is None:
				...
				try:
					...
					if clf is None or pre is None or not hasattr(clf, 'feature_importances_'):
						...
						sns.barplot(x=importances[sel], y=feat_names[sel], ax=ax)
...
with tabs[4]:
	...
	if df is None or df.empty:
		...
		with c6:
			color_by = st.selectbox("Color by (optional)", options=['(none)'] + cats, index=0)
		...
		if x_axis in dfx.columns and y_axis in dfx.columns:
			if color_by != '(none)' and color_by in dfx.columns and color_by != x_axis:
				# Group by x and color
				grp = dfx.groupby([x_axis, color_by])[y_axis]
				if agg_fn == 'sum':
					s = grp.sum().reset_index(name=y_axis)
				elif agg_fn == 'mean':
					s = grp.mean().reset_index(name=y_axis)
				elif agg_fn == 'median':
					s = grp.median().reset_index(name=y_axis)
				elif agg_fn == 'count':
					s = grp.count().reset_index(name=y_axis)
				# pick top N x categories by total across colors
				topx = s.groupby(x_axis)[y_axis].sum().sort_values(ascending=False).head(topn).index
				s = s[s[x_axis].isin(topx)]
			else:
				...
				if isinstance(s, pd.Series):
					s = s.reset_index(name=y_axis)
				else:
					s = s.reset_index()

			# Handle case with no data
			if s is None or (isinstance(s, pd.DataFrame) and s.empty):
				st.info("No data available after applying filters and aggregation.")
				base = None
			else:
				# Build Altair chart using selected chart type and color
				is_x_numeric = pd.api.types.is_numeric_dtype(s[x_axis]) if x_axis in s.columns else False
				if chart_type == 'Bar':
					enc = {
					...
					if color_by != '(none)' and color_by in s.columns:
						enc["color"] = alt.Color(f"{color_by}:N", title=color_by.replace('_', ' ').title())
					base = alt.Chart(s).mark_bar().encode(**enc).properties(height=400)
				else:
					x_type = 'Q' if is_x_numeric else 'N'
					mark = alt.MarkDef(type='line') if chart_type == 'Line' else alt.MarkDef(type='area')
					enc = {
						"x": alt.X(f"{x_axis}:{x_type}", title=x_axis.replace('_', ' ').title()),
						"y": alt.Y(f"{y_axis}:Q", title=y_axis.replace('_', ' ').title()),
						"tooltip": list(s.columns),
					}
					if color_by != '(none)' and color_by in s.columns:
						enc["color"] = alt.Color(f"{color_by}:N", title=color_by.replace('_', ' ').title())
					base = alt.Chart(s).mark_line() if chart_type == 'Line' else alt.Chart(s).mark_area()
					base = base.encode(**enc).properties(height=400)
			...
			if st.session_state.chart_specs:
				...
				for i, spec in enumerate(st.session_state.chart_specs):
					...
					if df_spec.empty:
						cols[i % 2].info("Empty chart")
						continue
					ctype = spec.get('chart_type', 'Bar')
					cby = spec.get('color_by', None)
					is_x_numeric = pd.api.types.is_numeric_dtype(df_spec[spec['x_axis']]) if spec['x_axis'] in df_spec.columns else False
					if ctype == 'Bar':
						enc = {
						...
						if cby and cby in df_spec.columns:
							enc["color"] = alt.Color(f"{cby}:N", title=cby.replace('_',' ').title())
						chart = alt.Chart(df_spec).mark_bar().encode(**enc).properties(height=300)
					else:
						x_type = 'Q' if is_x_numeric else 'N'
						enc = {
							"x": alt.X(f"{spec['x_axis']}:{x_type}", title=spec['x_axis'].replace('_',' ').title()),
							"y": alt.Y(f"{spec['y_axis']}:Q", title=spec['y_axis'].replace('_',' ').title()),
							"tooltip": list(df_spec.columns),
						}
						if cby and cby in df_spec.columns:
							enc["color"] = alt.Color(f"{cby}:N", title=cby.replace('_',' ').title())
						chart = alt.Chart(df_spec).mark_line() if ctype == 'Line' else alt.Chart(df_spec).mark_area()
						chart = chart.encode(**enc).properties(height=300)
